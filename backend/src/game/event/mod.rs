...
use crate::game::manager::obj_manager::{ObjManager, PlaceBombPayload};
...
use crate::ws::message::{
    make_ws_msg_game_state_changed, make_ws_msg_time_sync_pong, SendBombExplodePayload,
    SendCreateItemPayload, SendGameOverPayload, SendGameStateChangedPayload, ClientGenerateBombPayload,
    GridPos, SendItemEatenPayload, SendPlayerDiePayload, ClientMovePayload, SendRemoveFirePayload,
    SendRemoveItemPayload, SendTimeSyncPongPayload,
};
...
impl EventManager {
    pub fn new(map_data: &MapData, players: &[ClientDataForState]) -> Self {
        ...
        let obj_manager = ObjManager::new(map_manager, man_objs);
    ...
    pub fn consume_commands(
        ...
        let mut changes = SendGameStateChangedPayload::default();

        // ── 1. Handle incoming commands ───────────────────────────────────────
        while let Some(cmd) = commands.pop_front() {
            match cmd {
                GameCommand::PlayerMove(p) => self.handle_receive_player_move_event(    p, &mut changes),
                GameCommand::GenerateBomb(p) => self.handle_receive_generate_bomb_event(p, &mut changes),
                GameCommand::PlayerDisconnected(cid) => {
                    self.handle_receive_player_disconnected_event(cid, &mut changes)
                }
        ...
        for (cx, cy) in removed_fires {
            changes.removed_fires.push(SendRemoveFirePayload { x: cx as i32, y: cy as i32 });
        }
        ...
        for pos in &explode_data.destroy_item_positions {
            changes
                .removed_items
                .push(SendRemoveItemPayload { x: pos.index_x as i32, y: pos.index_y as i32 });
        }
        ...
        for player in &mut self.obj_manager.players {
            ...
            if fire_cells.contains(&(tx, ty)) {
                ...
                changes
                    .player_deaths
                    .push(SendPlayerDiePayload { man_key: player.man_sprite_key.clone() });
            }
        }
    ...
    fn handle_receive_player_move_event(&mut self, payload: ClientMovePayload, changes: &mut SendGameStateChangedPayload) {
        let is_alive = self
            .obj_manager
            .players
            .iter()
        ...
        {
            changes.player_moves.push(ClientMovePayload {
                man_key: mv.man_key,
                new_x: mv.new_x,
                new_y: mv.new_y,
                dir: mv.dir,
    ...
    fn handle_receive_generate_bomb_event(
        &mut self,
        payload: ClientGenerateBombPayload,
        changes: &mut SendGameStateChangedPayload,
    ) {
        let is_alive = self
            .obj_manager
            .players
        ...
        if payload.index_x < 0 || payload.index_y < 0 {
            return;
        }
        if let Some(placed) = self.obj_manager.handle_player_place_bomb(&payload) {
            changes.new_bombs.push(payload.clone());
        }
    }

    fn handle_receive_player_disconnected_event(
        &mut self,
        client_id: u32,
        changes: &mut SendGameStateChangedPayload,
    ) {
        if let Some(man_key) = self.client_man_map.get(&client_id).cloned() {
            ...
            changes.player_deaths.push(SendPlayerDiePayload { man_key });
        }
    }
    ...
    fn handle_time_sync_ping(&self, client_id: u32, sent_at: i64, state: &AppState) {
        ...
        let msg = Message::Text(
            make_ws_msg_time_sync_pong(SendTimeSyncPongPayload { sent_at, to }).into(),
        );
    ...
    fn check_game_over(&mut self, changes: &mut SendGameStateChangedPayload) {
        if self.game_over {
            return;
        }
